...
-- (Do not use any tactics!)
def exercise2:  a * (b * c) = (a * b) * c := (Nat.mul_assoc a b c).symm
...
def exercise1tactics : (a * b) * c = a * (b * c) := by rw [Nat.mul_assoc a b c]
...
def exercise2tactics:  a * (b * c) = (a * b) * c := by rw [(Nat.mul_assoc a b c).symm]
